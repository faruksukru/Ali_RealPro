...
import getAccountChartData from '@salesforce/apex/AccountChartController.getAccountChartData';
import sendData from '@salesforce/apex/AccountChartController.sendData';
import getInventoryData from '@salesforce/apex/AccountChartController.getInventoryData';
...
export default class AccountBarChart extends LightningElement {
    ...
    handleCardClick(event) {
        this.accountSource = event.currentTarget.dataset.id;
    ...
    handleSend() {
        if (this.validate()) {
            this.isProcessing = true;
                ...
                sendData({inventory: colums, leadSource: this.accountSource, channel: this.channelSelected, message: this.message, template: this.templateSelected, freeText: this.freeText})
                .then(result => {
                    if (result) {
    ...
    validate() {
        if(this.channelSelected == 'SMS' && !this.message) {
            this.showToast('ERROR', 'Please enter your message.');
            return false;
